import { Contract, Result, TransactionResponse, Transaction } from 'ethers'
...
export const getAbi = async (address, etherscan_key) => {  
...
export const buildContract = (address, etherscan_key) => {
    return new Promise(async (resolve, reject) => {
        ...
        if(!contract instanceof Contract) reject('Contract class failed. Please report to the developer along with the contract address you tried to use.');
...
export const parsePuts = async (contract, abi) => {

    if(!contract || !contract instanceof Contract) throw new Error(`parsePuts cannot be called without a proper ethers/contract instance`);
...
export const beautify = (types, inputs) => {
...
export const parseTransaction = (contract, transaction) => {

    if(!contract || !contract instanceof Contract) throw new Error(`Contract passed to parseTransactions must be an instance of ethers/contract`);

        if(!transaction || (!transaction instanceof Transaction && !transaction instanceof TransactionResponse) === true) throw new Error(`Transaction must be passed and you must ensure it i`)
